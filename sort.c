...
void	convert_to_index(t_list_ps *stack, int *values, int v_len)
{
	...
	sorted_node = sorted_stack->tail;
	index = v_len - 1;
	while (sorted_node && index >= 0)
	{
		stack_runner = stack->head;
		while (sorted_node && stack_runner->value != sorted_node->value)
			stack_runner = stack_runner->next;
		stack_runner->value = index--;
